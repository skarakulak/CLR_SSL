...
class ResNet(nn.Module):
    ...
    def __init__(
        ...
        num_clust=2000, dp=0, drop2d=0,hier_smax=0,num_of_paths=0
        ):
        super(ResNet, self).__init__()
        self.inplanes = 64
        self.dp = dp
        self.drop2d=drop2d
        self.hier_smax = hier_smax
        ...
        if hier_smax:
            self.hier_fc = nn.Linear(512 * block.expansion, num_of_paths)
    ...
    def forward(self, x, add_eps = None, return_c_dist=False, return_latent=False, return_hier_smax=False, return_id=False):
        c_dist = 0
        z_list = []
        id_list = []
        if self.dp and 0<self.dp<1: x = self.drop_input(x)
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool(x)
        ...
        if return_id: id_list.append(self.get_clusterid(x,0))

        # z = x.detach()
        # z = z[0].cpu()
        # img = transforms.ToPILImage()(z)
        ...
        if return_latent: z_list.append(x.detach())
        if return_id: id_list.append(self.get_clusterid(x,1))
        ...
        if return_latent: z_list.append(x.detach())
        if return_id: id_list.append(self.get_clusterid(x,2))



        if self.dp and 0<self.dp<1: x = self.drop_fc(x)
        y_hat = self.fc(x)
        if return_hier_smax: y_hat_ent=self.hier_fc(x)

        if (not return_c_dist) and (not return_latent) and (not return_hier_smax):
            return y_hat
        return_obj = [y_hat]
        if return_latent: return_obj.append(z_list)
        if return_c_dist: return_obj.append(c_dist)
        if return_hier_smax: return_obj.append(y_hat_ent)
        if return_id: return_obj.append(id_list)

        return tuple(return_obj)



def resnet18(pretrained=False, num_clust=2000, dp=0, drop2d=0, hier_smax=0, num_of_paths=0, **kwargs):
...
def resnet34(pretrained=False, num_clust=2000,dp=0, drop2d=0, hier_smax=0, num_of_paths=0, **kwargs):
