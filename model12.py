...
def train(
    ...
    if epoch <= 45:
        ...
        with torch.no_grad():
            ...
            for input_sup,y in sup_loader:
                ...
                output_sup, latent_sup, logvar_sup, c_dist_sup, cluster_sup, c_softmax_sup = model(input_sup, add_eps=False, return_c_dist=True)
    ...
    for i, ((input_sup, target_sup), (input_unsup,target_unsup)) in enumerate(zip(sup_loader,unsup_loader)):
        ...
        # compute output
        output_sup, mu_sup, logvar_sup, c_dist_sup, cluster_sup, c_softmax_sup = model(input_sup, add_eps=True, return_c_dist=True)
        output_unsup, mu_unsup, logvar_unsup, c_dist_unsup, cluster_unsup, c_softmax_unsup = model(input_unsup, add_eps=True, return_c_dist=True)
