...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, num_clust=2000):
        ...
        self.cl_centers = torch.Tensor(num_clust, 512)
    ...
    def reparameterise(self, mu, logvar, add_eps, return_c):
        if return_c or add_eps: 
            ...
            c_softmax = F.softmax(((x_k-c_k)**2).sum(2),dim=1)
        if add_eps:
        ...
        if return_c:
            return mu, c_dist, c_min, c_softmax
        else:
            return mu
    ...
    def forward(self, x, add_eps = True, return_c_dist=False):
        ...
        x = self.avgpool(x).view(x.size(0), 2, 512)
        mu, logvar = x[:,0,:], x[:,1,:]

        if return_c_dist: 
            z, c_dist, c_min, c_softmax = self.reparameterise(mu, logvar, add_eps, return_c_dist)
            return self.fc(z), mu, logvar, c_dist, c_min, c_softmax
        else: 
