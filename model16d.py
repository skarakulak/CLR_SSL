...
def train(
    ...
    for i, ((input_sup, target_sup), (input_unsup,target_unsup)) in enumerate(zip(sup_loader,unsup_loader)):
        ...
        # compute output
        if args.hier_softmax_entropy:
            output_sup, z_list, c_dist_sup, out_s_hsmx, id_list = model(input_sup, return_latent=True, return_c_dist=True, return_hier_smax=True, return_id=True)
            output_unsup, z_list, c_dist_unsup, out_us_hsmx, id_list = model(input_unsup, return_latent=True, return_c_dist=True, return_hier_smax=True, return_id=True)
            ...
            for layer_num, (z_batch, id_batch) in enumerate(zip(z_list, id_list)):
                for sample_num, id in enumerate(id_batch):
                    if id == 1 and imgcount < 5:
                        z = z_batch[sample_num].cpu() #get the corresponding image from the batch
                        img = transforms.ToPILImage()(z)
                        img.save('output{}_{}-{}.png'.format(epoch,layer_num,sample_num))
                        imgcount+=1

                    # write_to_log(log_path, 'Cluster ID: {}'.format(id))
...
def train_and_val(args):
    ...
    if args.hpc == 'prince':
        ...
        data_path = f'/scratch/{args.user}/unsupervised_learning_competition/ssl_data_96'
    else:
