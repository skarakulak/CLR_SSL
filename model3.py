...
def train(sup_loader, unsup_loader, model, criterion, optimizer, epoch, args, device, log_path):
    ...
    for i, ((input_sup, target_sup), (input_unsup,target_unsup)) in enumerate(zip(sup_loader,unsup_loader)):
        ...
        loss_uncertainty = F.relu(output_unsup_var.sum() - output_sup_var.sum() ,inplace=True)
        loss_ent = torch.distributions.Categorical(F.softmax(output_unsup,dim=1)).entropy().mean() 
        loss =(1-args.coef_unsup_ent_loss)*loss_cse + args.coef_unsup_ent_loss*loss_ent + args.coef_uncertainty_loss*loss_uncertainty
