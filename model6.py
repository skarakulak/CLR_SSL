...
def train(sup_loader, unsup_loader, model, criterion, optimizer, epoch, args, device, log_path):
    ...
    for i, ((input_sup, target_sup), (input_unsup,target_unsup)) in enumerate(zip(sup_loader,unsup_loader)):
        ...
        # compute output
        output_sup, loss_sup_cdist = model(input_sup, return_c_dist=True)
        output_unsup, loss_unsup_cdist = model(input_unsup, return_c_dist=True)
        
        
        loss_cse = criterion(output_sup, target_sup)
        cdist_multiplier = args.coef_unsup_cdist_loss if epoch > 21 else args.coef_unsup_cdist_loss * (10**(-11+epoch//2))
